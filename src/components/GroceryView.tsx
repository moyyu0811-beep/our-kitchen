...
import { ShoppingCart, Package, History, Plus, CheckCircle2, Trash2, AlertTriangle } from 'lucide-react';
import { format } from 'date-fns';

type Tab = 'shopping' | 'inventory' | 'history';

export const GroceryView = () => {
  const { groceryList, inventory, purchaseHistory, addGroceryItem, checkOffGrocery, removeGroceryItem, ranOutInventory } = useStore();
  ...
  const tabs = [
    { id: 'shopping' as Tab, label: 'Shopping List', icon: <ShoppingCart size={18} />, count: groceryList.length },
    { id: 'inventory' as Tab, label: 'Inventory', icon: <Package size={18} />, count: inventory.length },
    { id: 'history' as Tab, label: 'History', icon: <History size={18} />, count: purchaseHistory.length },
  ];

  return (
    <div className="container" style={{ padding: '2rem 1.5rem' }}>
      <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
        <h2 style={{ fontSize: '2rem', marginBottom: '0.5rem' }}>Grocery</h2>
        <p style={{ color: 'var(--text-secondary)' }}>Shared across all family devices</p>
      </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'center', gap: '0.75rem', marginBottom: '2rem', flexWrap: 'wrap' }}>
        {tabs.map(t => (
          <button key={t.id} onClick={() => setTab(t.id)} className="hover-lift" style={{
            ...
            fontWeight: 600, border: '1px solid var(--border-color)', transition: 'all 0.2s'
          }}>
            {t.icon} {t.label}
            {t.count > 0 && (
              <span style={{
                background: tab === t.id ? 'rgba(255,255,255,0.25)' : 'var(--accent-color)',
                color: 'white', borderRadius: '999px', padding: '0 8px', fontSize: '0.75rem', fontWeight: 700
              }}>{t.count}</span>
            )}
          </button>
        ))}
      ...
      <div className="glass-panel" style={{ padding: '2rem', maxWidth: '600px', margin: '0 auto' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                {groceryList.map((item: GroceryItem) => (
                  <div key={item.id} className="glass" style={{
                    ...
                    <button onClick={() => checkOffGrocery(item)} style={{
                      width: 28, height: 28, borderRadius: '50%', flexShrink: 0,
                      border: '2px solid var(--accent-color)', display: 'flex',
                      alignItems: 'center', justifyContent: 'center', color: 'var(--accent-color)',
                      transition: 'all 0.2s', background: 'transparent'
                    }} className="hover-lift" title="Mark as purchased">
        ...
        {tab === 'inventory' && (
          <>
            ...
            {inventory.length === 0 ? (
              <div style={{ textAlign: 'center', color: 'var(--text-muted)', padding: '2rem 0' }}>
                No items in inventory yet. Check off items from your shopping list!
              </div>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                {inventory.map((item: InventoryItem) => (
                  <div key={item.id} className="glass" style={{
                    ...
                    <button onClick={() => ranOutInventory(item)} className="hover-lift active-scale" style={{
                      ...
                      fontWeight: 600, fontSize: '0.8rem', border: '1px solid rgba(239,68,68,0.2)'
                    }}>
